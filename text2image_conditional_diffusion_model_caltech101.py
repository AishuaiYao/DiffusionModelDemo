...
def train_diffusion_model(model, dataloader, num_steps, beta, num_epochs, lr):
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (data, _) in enumerate(dataloader):
            data = data.to(device)
            # 这里暂时使用类别名作为文本描述，可根据实际情况修改
            class_names = [dataloader.dataset.classes[label] for label in range(len(data))]
...
# 下载并加载 Caltech 101 数据集
train_dataset = ImageFolder(root='./data/caltech101/101_ObjectCategories', transform=transform)
